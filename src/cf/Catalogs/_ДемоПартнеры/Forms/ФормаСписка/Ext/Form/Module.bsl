...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УдалениеПомеченныхОбъектов.УстановитьПометкуКомандыПоказатьПомеченные(ЭтотОбъект, Элементы.Список,
		Элементы.ПоказатьПомеченныеНаУдаление);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись__ДемоПартнеры" И Элементы.Список.ТекущаяСтрока = Источник Тогда
...
Процедура ЗаменитьИУдалить(Команда)
	...
	ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
		"_ДемоИнформацияКлиента._ДемоНоменклатура.ЗаменитьИУдалить.Нажатие", 1);

...
Функция Подключаемый_ПроверитьСовпадениеМодулейУправляемогоИОбычногоПриложений()
	...
	// Указанных ниже событие нет в модуле обычного приложения, перед сравнением удаляем их.
	ИсключаемыеСобытия = Новый Массив;
	ИсключаемыеСобытия.Добавить("ПриГлобальномПоиске");
	ИсключаемыеСобытия.Добавить("ПриВыбореРезультатаГлобальногоПоиска");
	ИсключаемыеСобытия.Добавить("ПриВыбореДействияРезультатаГлобальногоПоиска");
	ИсключаемыеСобытия.Добавить("ОбработкаПереходаПоНавигационнойСсылке");
	
	Текст = ТекстФайлаУправляемогоПриложения.ПолучитьТекст();
	Для Каждого Событие Из ИсключаемыеСобытия Цикл
		ВырезатьСобытие(Текст, Событие);
	КонецЦикла;
	ТекстФайлаУправляемогоПриложения.УстановитьТекст(Текст);
...
#Область ВспомогательныеПроцедурыИФункцииПроверок
...
Процедура ОпределитьОшибкиУсловныхВызовов(ФайлМодуля, Знач СтрокаТекста, Ошибки, СписокИспользуемыхПодсистем, СтрокаУсловногоВызова)
	...
	Если ПодсистемыИсключенияПроверкиУсловныхВызовов = Неопределено Тогда
		...
		ПодсистемыИсключенияПроверкиУсловныхВызовов.Добавить("ПлюсСервис");
	КонецЕсли;
...
&НаСервере
Процедура ВырезатьСобытие(Текст, Событие)
	
	СтрокаПоиска  = СтрШаблон("Процедура %1", Событие);
	НачалоСобытия = СтрНайти(Текст, СтрокаПоиска);
	Если НачалоСобытия = 0 Тогда
		Возврат;
	КонецЕсли;
	КонецСобытия = СтрНайти(Текст, "КонецПроцедуры", , НачалоСобытия);
	
	Текст = Лев(Текст, НачалоСобытия - 2) + Сред(Текст, КонецСобытия + СтрДлина("КонецПроцедуры") + 1);

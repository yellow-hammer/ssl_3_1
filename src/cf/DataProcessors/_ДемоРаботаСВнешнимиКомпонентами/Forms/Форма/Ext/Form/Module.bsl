...
Процедура ПолучитьИнформацию(Команда)
	...
	ОчиститьСообщения();
	ПолучитьИнформациюОКомпонентеНаСервере();
...
&НаСервере
Процедура ПолучитьИнформациюОКомпонентеНаСервере()
	
	Результат = ВнешниеКомпонентыСервер.ИнформацияОКомпоненте(Идентификатор,

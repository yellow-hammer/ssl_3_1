...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru = 'План выполнения запроса недоступен по причине:'")
			+ Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Сообщение.Сообщить();
		Элементы.ГруппаПланВыполненияЗапроса.Доступность = Ложь;
	КонецПопытки;
...
Процедура ЗаполнитьИзXML(Команда)
	...
	Если Лев(СокрЛ(ТекущийЗапрос.Текст), 10) = "<Structure" Тогда

...
Процедура ВыгрузитьТекущуюКонфигурациюВФайлы(ПапкаВыгрузки, ИмяПользователя = Неопределено, Пароль = Неопределено)
	...
	Иначе
		...
		Если Свойства.ТипАлгоритмаХешированияПароля <> Null Тогда
			ПарольСовпадает = ПользователиСлужебный.ПроверитьСоответствиеПароляПользователяСохраняемомуЗначению_8_3_26(Пароль, ПользовательИБ);
		Иначе
		...
		Если ВидДоступа <> "Условие"
		   ...
		   И ВидДоступа <> "ПравоИзменения" Тогда
			...
			Если ВидДоступа = "Объект" Или ВидДоступа = "НастройкиПрав" Тогда
				...
				|ГДЕ
				|	ЛОЖЬ"; // @query-part-1, @query-part-2

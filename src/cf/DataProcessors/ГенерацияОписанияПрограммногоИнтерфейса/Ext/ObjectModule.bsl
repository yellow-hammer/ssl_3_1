...
Перем КомментарийПрочитан, ЧтениеПараметров, ПолноеИмяОбъекта, ИмяОбласти, ТипМодуля,
...
Процедура СформироватьПрограммныйИнтерфейс() Экспорт
	...
	ФайлыМодулей = НайтиФайлы(КаталогВыгрузки + ПолучитьРазделительПути(), "*bsl", Истина);
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		Если СтрНайти(ВРег(ФайлМодуля.Путь), "\FROM\") > 0
		 ИЛИ СтрНайти(ВРег(ФайлМодуля.Путь), "_ДЕМО") Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ВРег(ФайлМодуля.ИмяБезРасширения) = "RECORDSETMODULE"
		   И НЕ ВРег(ФайлМодуля.ИмяБезРасширения) = "MANAGERMODULE"
		   И НЕ ВРег(ФайлМодуля.ИмяБезРасширения) = "VALUEMANAGERMODULE"
		   И НЕ ВРег(ФайлМодуля.ИмяБезРасширения) = "OBJECTMODULE"
		   И НЕ ВРег(ФайлМодуля.ИмяБезРасширения) = "MODULE" Тогда
			Продолжить;
		КонецЕсли;
		
		ТипМодуля = СоответствиеРусскихИАнглийскихИмен()[ФайлМодуля.ИмяБезРасширения];
		...
		Иначе
			ДоступностьНачальное = ПолучитьТаблицуИнструкцийПрепроцессора(ТекстМодуля);
			ДоступностьМетода	 = ДоступностьНачальное;
		КонецЕсли;
	...
	КонецЕсли;
	СтруктураПрограммногоИнтерфейса.Сортировать("Подсистема, Размещение, ПолноеИмяОбъекта, Область, ИмяМетода");
...
Функция НоваяСтруктураПрограммногоИнтерфейса()
	...
	СтруктураПрограммногоИнтерфейса.Колонки.Добавить("Тип");
	СтруктураПрограммногоИнтерфейса.Колонки.Добавить("ТипМодуля");
	СтруктураПрограммногоИнтерфейса.Колонки.Добавить("ИмяОбъекта");
...
Процедура ПрочитатьКомментарийИМетод(Строка, ТекстовыйДокумент, Итератор, Рекурсия = Ложь)
	...
	ПозицияСтрокиАПК = СтрНайти(ВРег(Строка), "// АПК");
	Если ПозицияСтрокиАПК > 0 Тогда
		ДлинаСтрокиАПК = СтрДлина(Строка) - (СтрДлина(Строка) - ПозицияСтрокиАПК + 1);
		Строка = Лев(Строка, ДлинаСтрокиАПК);
		СледующаяСтрокаАПК = ТекстовыйДокумент.ПолучитьСтроку(Итератор + 1);
		ЗаглавнаяСтрочнаяБуква = Лев(СледующаяСтрокаАПК, 4);
		Если НРег(ЗаглавнаяСтрочнаяБуква) = ЗаглавнаяСтрочнаяБуква И Прав(СледующаяСтрокаАПК, 1) = "." Тогда
			Итератор = Итератор + 1;
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрНайти(ВРег(Строка), "@SKIP") > 0 Тогда
		Возврат;
	КонецЕсли;
	...
	РаздельноеИмяОбъекта = СтрРазделить(ПолноеИмяОбъекта, ".");
	
	Если РаздельноеИмяОбъекта.Количество() > 1 Тогда
		СтруктураМетода.Тип = РаздельноеИмяОбъекта[0];
		СтруктураМетода.ИмяОбъекта = РаздельноеИмяОбъекта[1];
		СтруктураМетода.СинтаксисВызова = ПодготовитьПримерВызова(СтруктураМетода, СинтаксисВызоваМетода);
	Иначе
		...
		Иначе
			СтруктураМетода.СинтаксисВызова = СтруктураМетода.СинтаксисВызова;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПрочитатьМетод(Строка, ТекстовыйДокумент, Итератор, ОшибкаЧтенияМетода, Рекурсия = Ложь)
	...
	Пока Не МетодПрочитан Цикл
		...
		Если ТекстовыйДокумент.КоличествоСтрок() <= Итератор Тогда
			ОшибкаЧтенияМетода = Истина;
			Возврат;
		КонецЕсли;
		Если ОшибкаЧтенияМетода Тогда
...
Процедура УстановитьПеременные()
	...
	СтруктураМетода.Вставить("Тип", "");
	СтруктураМетода.Вставить("ТипМодуля", ТипМодуля);
	СтруктураМетода.Вставить("ИмяОбъекта", "");
...
Функция ИмяМенеджераОбъектаПоТипу(ТипОбъекта)
	...
	Если ТипОбъектаИМенеджер = Неопределено Тогда
		...
		ТипОбъектаИМенеджер.Вставить("Константа", "Константы.");
...
Процедура ЗаполнитьСоответствиеОбъектовПодсистемамПоВыгрузке(Соответствие)
	...
	Для Каждого ФайлПодсистемы Из ФайлыПодсистем Цикл
		...
		Пока Истина Цикл
			...
			МассивСоставаЭлемента = СтрРазделить(ТекЭлементСостава.ТекстовоеСодержимое, ".");
			
			Если МассивСоставаЭлемента.Количество() > 0 Тогда
				ТипМетаданных = МассивСоставаЭлемента[0];
				ТипМетаданныхПереведенный = СоответствиеРусскихИАнглийскихИмен[ТипМетаданных];
				ИмяЭлемента = СтрЗаменить(ТекЭлементСостава.ТекстовоеСодержимое, ТипМетаданных, ТипМетаданныхПереведенный);
...
Функция ПолучитьТаблицуИнструкцийПрепроцессора(Знач ТекстМодуля)
	...
	Если АнглийскаяВерсия Тогда
		МассивПоиска.Добавить(ВРег("#if"));
		МассивПоиска.Добавить(ВРег("#Endif"));
		МассивТекстЕсли.Добавить(ВРег("#if"));
		МассивТекстКонецЕсли.Добавить(ВРег("#Endif"));
	Иначе
		МассивПоиска.Добавить(ВРег("#Если"));
		МассивПоиска.Добавить(ВРег("#КонецЕсли"));
	...
	Если СтрНайти(ТекстИнструкции, "Сервер") > 0 Тогда
		ДоступностьМетода.Добавить(СтрокаСервер());
	КонецЕсли;
	
	Если СтрНайти(ТекстИнструкции, "Тонкий") > 0 Тогда
		ДоступностьМетода.Добавить(СтрокаТонкийКлиент());
	КонецЕсли;
	
	Если СтрНайти(ТекстИнструкции, "Толстый") > 0 Тогда
		ДоступностьМетода.Добавить(СтрокаТолстыйКлиент());
	КонецЕсли;
	
	Если СтрНайти(ТекстИнструкции, "ВнешнееСоединение") > 0 Тогда
		ДоступностьМетода.Добавить(СтрокаВнешнееСоединение());
	КонецЕсли;
	
	ДоступностьМетода = СтрСоединить(ДоступностьМетода, ", ");
	
	Возврат ДоступностьМетода;
...
Функция СтрНайтиПоМассиву(Строка, МассивПоиска, НаправлениеДляПоиска = Неопределено,
	...
	Для Каждого ТекстПоиска Из МассивПоиска Цикл
		...
		Если ПоискСНачала Тогда
			СледуетПродолжатьПоиск = ТекущаяПозицияПоиска < ВременнаяПозицияПоиска;
		Иначе
			СледуетПродолжатьПоиск = ТекущаяПозицияПоиска > ВременнаяПозицияПоиска;
		КонецЕсли;
		
		Если (ВременнаяПозицияПоиска = 0)
			ИЛИ СледуетПродолжатьПоиск Тогда
...
Функция ПодготовитьПримерВызова(СтруктураМетода, СинтаксисВызоваМетода)
	
	ИмяМенеджераОбъекта = ИмяМенеджераОбъектаПоТипу(СтруктураМетода.Тип) + СтруктураМетода.ИмяОбъекта;
	
	Если СтруктураМетода.Тип = "Обработка"
		И СтруктураМетода.ТипМодуля = "МодульОбъекта" Тогда
		
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1ОбработкаОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("ОбработкаОбъект = %1.Создать();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
		
	ИначеЕсли СтруктураМетода.Тип = "Отчет"
			И СтруктураМетода.ТипМодуля = "МодульОбъекта" Тогда
			
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1ОтчетОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("ОтчетОбъект = %1.Создать();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
		
	ИначеЕсли СтруктураМетода.Тип = "БизнесПроцесс"
			И СтруктураМетода.ТипМодуля = "МодульОбъекта" Тогда
			
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1БизнесПроцессОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("БизнесПроцессОбъект = %1.СоздатьБизнесПроцесс();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
			
	ИначеЕсли СтруктураМетода.Тип = "Константа"
			И СтруктураМетода.ТипМодуля = "МодульМенеджераЗначения" Тогда
			
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1КонстантаОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("КонстантаОбъект = %1.СоздатьМенеджерЗначения();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
			
	ИначеЕсли СтруктураМетода.Тип = "Документ"
			И СтруктураМетода.ТипМодуля = "МодульОбъекта" Тогда
			
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1ДокументОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("ДокументОбъект = %1.НайтиПоНомеру().ПолучитьОбъект();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
		
	ИначеЕсли НЕ СтруктураМетода.Тип = "ОбщийМодуль"
			И СтруктураМетода.ТипМодуля = "МодульОбъекта" Тогда
			
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1Объект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("Объект = %1.НайтиПоКоду().ПолучитьОбъект();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
			
	ИначеЕсли СтруктураМетода.ТипМодуля = "МодульНабораЗаписей" Тогда
		
		ТекстОбработчика = ?(СинтаксисВызоваМетода.ЭтоФункция, "Результат = ", "");
		ПримерОбработки = СтрШаблон("%1РегистрОбъект.%2();", ТекстОбработчика, СтруктураМетода.ИмяМетода);
		СтруктураМетода.СинтаксисВызова = СтрШаблон("РегистрОбъект = %1.СоздатьНаборЗаписей();%2%3",
			ИмяМенеджераОбъекта, Символы.ПС, ПримерОбработки);
	...
	Возврат СтруктураМетода.СинтаксисВызова;

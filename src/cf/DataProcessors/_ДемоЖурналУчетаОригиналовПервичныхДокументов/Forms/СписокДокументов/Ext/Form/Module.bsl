...
&НаКлиенте
Процедура ГруппировкаПоДокументам(Команда)
	
	Если Элементы.ГруппировкаПоДокументам.Пометка <> Истина Тогда
...
&НаКлиенте
Процедура ГруппировкаПоПечатнымФормам(Команда)
	
	Если Элементы.ГруппировкаПоПечатнымФормам.Пометка <> Истина Тогда 
		ОтображениеСписка = "ПоПечатнымФормам";
		ОтображениеСпискаПриИзмененииНаСервере();
	КонецЕсли;
...
Процедура ВосстановитьНастройки()
	...
	Иначе // Если нет сохраненных настроек заполняем список для отбора по виду документов и настройки по умолчанию
...
Процедура ЗаполнитьПервоначальныйСписокВыбораВидовДокумента() 
	...
	ТипыДокументов = Новый Массив; // Массив Из ОбъектМетаданных
	...
	ИдентификаторыОбъектовМетаданных = ОбщегоНазначения.ИдентификаторыОбъектовМетаданных(ТипыДокументов, Ложь);
	Для каждого ТипДокумента Из ТипыДокументов Цикл
		ИдентификаторОбъектаМетаданных = ИдентификаторыОбъектовМетаданных[ТипДокумента.ПолноеИмя()];
		Если ИдентификаторОбъектаМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Отбор.Представление = ТипДокумента.Синоним;
		НайденныеСтроки = ТаблицаВидыДокументов.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() = 0 Тогда
			НоваяСтрока = ТаблицаВидыДокументов.Добавить();
			НоваяСтрока.ВидДокумента = ИдентификаторОбъектаМетаданных;
			НоваяСтрока.Представление = ТипДокумента.Синоним;
...
Процедура УстановитьОтборПоВидуДокумента(Результат)
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокДокументов,"ТипСсылки",
			ОтборВидыДокументов, ВидСравненияКомпоновкиДанных.ВСписке,,Истина);

		Если ОтборВидыДокументов.Количество() = КоличествоДокументов Или ОтборВидыДокументов.Количество()= 0 Тогда
			Элементы.ИнформационнаяНадписьОтбор.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Показаны <a href=""%1"">все документы журнала</a>'"),
				"НастроитьОтбор");
		Иначе	
			ТекстНадписи = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				НСтр("ru = ';Показан <a href>%1 документ журнала</a>;;
				|Показано <a href>%1 документа журнала</a>;Показано <a href>%1 документов журнала</a>;'"),
				ОтборВидыДокументов.Количество());
			Элементы.ИнформационнаяНадписьОтбор.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				СтрЗаменить(ТекстНадписи, "<a href>", "<a href=""%1"">"),
				"НастроитьОтбор");	
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ИнформационнаяНадписьОтбор.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Показаны <a href=""%1"">все документы журнала</a>'"), "НастроитьОтбор");
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокДокументов,
			"ТипСсылки", ОтборВидыДокументов, ВидСравненияКомпоновкиДанных.ВСписке,,Истина);
...
Процедура ОтображениеСпискаПриИзмененииНаСервере()
	...
	Если ОтображениеСписка = "ПоДокументам" Тогда
		...
		Элементы.ГруппировкаПоДокументам.Пометка = Истина;
		Элементы.ГруппировкаПоПечатнымФормам.Пометка = Ложь;
	ИначеЕсли ОтображениеСписка = "ПоПечатнымФормам" Тогда
		...
		Элементы.ГруппировкаПоПечатнымФормам.Пометка = Истина;
		Элементы.ГруппировкаПоДокументам.Пометка = Ложь;
	КонецЕсли;
